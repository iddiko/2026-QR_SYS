  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.4em] text-blue-600 dark:text-sky-300">{scope.name}</p>
          <h2 className="mt-2 text-3xl font-semibold text-slate-950 dark:text-white">대시보드</h2>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{session?.email ?? '로그인 정보 없음'} · 역할: {role}</p>
        </div>
      ...
      {(role === 'GUARD' || role === 'RESIDENT') && (
        <section className="rounded-3xl border border-slate-200/80 bg-white/70 p-6 backdrop-blur dark:border-white/10 dark:bg-slate-950/40">
          ...
          <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">경비/입주민 화면은 준비 중입니다</h3>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            현재는 관리자 기능 중심으로 개발 중입니다. 추후 경비/입주민용 메뉴와 QR 조회/발급 흐름을 연결합니다.
          </p>
        </section>
      )}
    </div>
